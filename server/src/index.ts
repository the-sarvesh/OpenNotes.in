      ...
      try {
        ...
        const orderCheck = await db.execute({
          sql: `SELECT o.id FROM orders o
                ...
                LIMIT 1`,
          args: [
            conversationId.split('_')[2], // listingId from ID
            userId, conversationId.split('_')[2], userId,
            conversationId.split('_')[1] === userId ? conversationId.split('_')[0] : conversationId.split('_')[1],
            conversationId.split('_')[2], userId
          ],
        });

        // Simpler fallback: just check if the conversationId format is valid and user is part of it
