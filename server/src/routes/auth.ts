...
// Internal testing only — try to send a test email and return the result.
router.get("/debug-email", async (req, res) => {
  ...
  try {
    console.log(`[Debug Email] Testing SMTP config with recipient: ${email}`);
    
    // Create a simple test email
    const mailOpts = {
      to: email,
      subject: "🔧 OpenNotes Production Email Test",
      html: `
        <h1>Debug Test</h1>
        <p>This is a manual test of the OpenNotes SMTP configuration from the production environment.</p>
        <ul>
    ...
    // Use await here to catch the result directly for the API response
    ...
    res.json({
      ...
      configUsed: {
        ...
        from: process.env.EMAIL_FROM || "OpenNotes <no-reply@opennotes.in>",
      }
    });
    ...
    res.status(500).json({
      ...
      tip: "If code is ETIMEDOUT, Render might be blocking Port 587. If code is EAUTH, check your App Password or Resend API key."
    });
  }
});
