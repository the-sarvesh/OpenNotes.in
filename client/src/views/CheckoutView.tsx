...
const SectionHeader: React.FC<{ icon: React.ReactNode; title: string }> = ({ icon, title }) => (
  <div className="flex items-center gap-3 mb-5">
    <div className="p-2.5 bg-primary/10 rounded-xl text-primary shrink-0">{icon}</div>
...
const FieldLabel: React.FC<{ children: React.ReactNode; required?: boolean }> = ({ children, required }) => (
  <label className="block text-[10px] font-black text-text-muted uppercase tracking-widest mb-2">
    {children}{required && <span className="text-red-500 ml-1 text-[9px]">(Required)</span>}
  </label>
);

const inputCls = "w-full px-4 py-3 bg-background border border-border rounded-xl text-sm text-text-main placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-primary transition-all";
  ...
  <div className="rounded-2xl border border-border overflow-hidden">
    <div className="grid grid-cols-2 divide-x divide-border">
      ...
      <div className="p-3.5 flex flex-col gap-0.5 bg-surface">
        ...
        <p className="text-[9px] text-text-muted">To seller · after inspecting notes</p>
      </div>
    </div>
...
export const CheckoutView: React.FC<CheckoutViewProps> = ({ cart, onSuccess, onBack }) => {
  ...
  const [couponResult, setCouponResult] = useState<{
    valid: boolean; message: string; originalFee: number; finalFee: number;
    feeWaived: boolean; discountType: string; discountValue: number;
  } | null>(null);
  ...
  const needsDelivery = activeCart.some(i => ["courier", "both", "Delivery"].includes(i.note.deliveryMethod));
  const needsMeetup = activeCart.some(i => ["in_person", "both", "Hand-to-Hand"].includes(i.note.deliveryMethod));
  ...
  const validateCoupon = useCallback(async () => {
    ...
    } catch { toast.error("Failed to validate coupon. Try again."); }
    finally { setCouponLoading(false); }
  }, [couponInput, total, rawPlatformFee]);

  const removeCoupon = () => { setCouponInput(""); setCouponResult(null); setAppliedCoupon(null); };
    ...
    try {
      const res = await apiRequest("/api/orders", {
        ...
        body: JSON.stringify({
          ...
          buyer_availability: `${collectionDate} (${collectionPeriod})`,
      ...
      if (res.ok) {
        ...
        else { clearCart(); toast.success("Order placed successfully!"); }
      ...
      <div className="grid lg:grid-cols-[1fr_340px] gap-6 lg:gap-8 items-start">
        ...
        <div className="space-y-4">
          ...
          <AnimatePresence mode="wait">
            ...
            {/* STEP 1 */}
            {step === 1 && (
              <motion.div key="step1" initial={{ opacity: 0, x: -12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: 12 }} transition={{ duration: 0.18 }} className="space-y-4">
                ...
                {PLATFORM_FEE_PERCENTAGE === 0 && (
                  <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-4 flex items-center gap-3.5">
                    ...
                    <div>
                      <p className="text-xs font-black text-emerald-600 uppercase tracking-widest mb-0.5">Launch Promo Active! 🎉</p>
                      <p className="text-[11px] text-emerald-800/70 dark:text-emerald-400/70 font-medium">Platform fees are waived for all BITSians for a limited time.</p>
                    </div>
                  </div>
                )}

                <PaymentSplitBanner platformFee={rawPlatformFee} cashAmount={total - rawPlatformFee} />
                ...
                <Card className="p-4 sm:p-6">
                  ...
                  <div className="space-y-5">
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        <FieldLabel>Your Region / City</FieldLabel>
                        <select value={buyerLocation} onChange={e => setBuyerLocation(e.target.value)} className={inputCls}>
                          {LOCATIONS.map(l => <option key={l}>{l}</option>)}
                        </select>
                      </div>
                      <div>
                        <FieldLabel>Preferred Hand-over Spot</FieldLabel>
                        <select value={buyerPreferredSpot} onChange={e => setBuyerPreferredSpot(e.target.value)} className={inputCls}>
                    ...
                    {buyerLocation === "Other (Manual)" && (
                      <div>
                        <FieldLabel required>Specify Your City / Region</FieldLabel>
                        <input type="text" value={customBuyerLocation} onChange={e => setCustomBuyerLocation(e.target.value)} placeholder="e.g. Mumbai, Kolkata, Pilani..." className={inputCls} />
                      </div>
                    )}

                    <div>
                      <FieldLabel>Specific Instructions / Meetup Details</FieldLabel>
                      <input type="text" value={buyerMeetupDetails} onChange={e => setBuyerMeetupDetails(e.target.value)} placeholder="e.g. hcl 126, cafe 3..." className={inputCls} />
                    </div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-3 gap-2">
                        {["Morning", "Afternoon", "Evening"].map(time => (
                          <label key={time} className={`flex items-center justify-center p-3 rounded-xl border cursor-pointer transition-all text-xs font-bold ${collectionPeriod === time ? "border-primary bg-primary/5 text-text-main" : "border-border bg-surface text-text-muted hover:border-primary/30"
                            }`}>
                            <input type="radio" name="availability" value={time} checked={collectionPeriod === time} onChange={e => setCollectionPeriod(e.target.value)} className="hidden" />
                            {time}
                          </label>
                        ))}
                      </div>
                    ...
                    <div>
                      <FieldLabel>Note to seller <span className="normal-case text-[9px] font-medium">(optional)</span></FieldLabel>
                      <textarea placeholder="Anything else you'd like to mention?" value={userNote} onChange={e => setUserNote(e.target.value)} rows={2}
            ...
            {/* STEP 2 */}
            {step === 2 && (
              <motion.div key="step2" initial={{ opacity: 0, x: 12 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -12 }} transition={{ duration: 0.18 }} className="space-y-4">

                <PaymentSplitBanner platformFee={platformFee} cashAmount={total - platformFee} />
                ...
                <Card className="p-4 sm:p-6 space-y-5">
                  ...
                  {/* Coupon */}
                  <div>
                    <FieldLabel><Tag className="h-3 w-3 inline mr-1" />Coupon Code (Optional)</FieldLabel>
                    {appliedCoupon && couponResult?.valid ? (
                      <div className="flex items-center justify-between p-3 bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200 dark:border-emerald-800 rounded-xl">
                        ...
                        <button onClick={removeCoupon} className="p-1.5 text-emerald-600 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-950/30 rounded-lg transition-colors">
                      ...
                      <div className="flex gap-2">
                        ...
                        />
                        <button onClick={validateCoupon} disabled={!couponInput.trim() || couponLoading}
                  ...
                  <label className="flex items-start gap-3 cursor-pointer p-3.5 bg-background rounded-xl border border-border hover:border-primary/40 transition-colors">
                    <div className="relative mt-0.5 shrink-0">
                      <input type="checkbox" checked={agreedToDelivery} onChange={e => setAgreedToDelivery(e.target.checked)}
        ...
        <div className="lg:sticky lg:top-6">
          <Card className="p-4 sm:p-5">
            <p className="text-[10px] font-black text-text-muted uppercase tracking-[0.2em] mb-4">Order Summary</p>

            {/* Items */}
            <div className="space-y-3 max-h-80 overflow-y-auto pr-0.5 mb-4 scrollbar-hide">
              {activeCart.map(item => (
                <div key={item.note.id} className="p-3 bg-background rounded-2xl border border-border/50">
                  <div className="flex gap-3 mb-2.5">
                    <div className="relative shrink-0">
                      <img src={item.note.image} className="h-14 w-14 rounded-xl object-cover border border-border" />
                      {item.quantity > 1 && (
                  ...
                  <div className="pt-2 border-t border-border/40 space-y-1.5">
                    <div className="flex items-start gap-2">
                      ...
                      <div className="min-w-0">
                        ...
                        <p className="text-[11px] font-bold text-text-main">{item.note.location || "BITS Pilani Campus"}</p>
                        {item.note.preferredMeetupSpot && (
                          <p className="text-[10px] text-primary font-black mt-0.5">📍 {item.note.preferredMeetupSpot}</p>
                        )}
                        {item.note.meetupLocation && (
                          <p className="text-[10px] text-text-muted mt-0.5 italic">({item.note.meetupLocation})</p>
                        )}
                      </div>
                    </div>
                    {item.note.materialType && (
                      <p className="text-[10px] text-text-muted italic pl-5 capitalize">
            ...
            <div className="border-t border-border pt-4 space-y-3">
              ...
              <div className={`flex justify-between items-center ${config.color}`}>
                ...
                <div className="flex flex-col items-end">
                  ...
                  <span className="text-sm font-black">{platformFee === 0 ? "FREE" : `₹${platformFee}`}</span>
                </div>
              </div>

              {/* Promo/coupon callout */}
              {(config.isPromo || couponResult?.valid) && (
                <div className={`p-3 rounded-xl border ${config.bgColor} ${config.borderColor} flex items-center gap-2.5`}>
                  <Info className={`h-3.5 w-3.5 shrink-0 ${config.color}`} />
                  <p className={`text-[10px] font-bold leading-relaxed ${config.color}`}>
                    {couponResult?.valid
                      ? `Coupon "${appliedCoupon}": ${couponResult.message}`
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div className="p-3 bg-background rounded-xl border border-border text-center">
                  <p className="text-[9px] font-black text-text-muted uppercase tracking-widest mb-1">Cash Meetup</p>
                  <p className="text-lg font-black text-text-main">{formatRupee(total - platformFee)}</p>
                </div>
              </div>
            </div>

            {/* Desktop CTAs */}
            <div className="hidden lg:flex flex-col gap-3 mt-5 pt-5 border-t border-border">
              {step === 1 ? (
                ...
                <>
                  <button onClick={goToStep1}
                    className="w-full py-3 bg-surface hover:bg-background text-text-muted rounded-xl font-bold text-xs uppercase tracking-widest transition-all border border-border active:scale-[0.98]">
                    ← Back to Meetup
                  </button>
                  <button onClick={handlePlaceOrder} disabled={loading || !agreedToDelivery}
        ...
        <div className="flex items-center justify-between mb-3">
          ...
          {step === 2 && (
            <span className="text-[10px] text-text-muted font-semibold">
              + {formatRupee(total - platformFee)} cash
            </span>
          )}
        </div>
